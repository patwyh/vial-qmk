...
void keyboard_post_init_user(void) {
    ...
    testimg = qp_load_image_mem(gfx_my_image);
    if (testimg != NULL) {
...
//     // Convert layer number to a string for display
//     char layer_str[16];
//     snprintf(layer_str, sizeof(layer_str), "Layer: %u", layer);

//     // Clear previous text area (x, y, width, height, R, G, B, filled)
//     qp_rect(display, 10, 10, 240, 30, 0, 0, 0, true);
    
//     // Draw the text (device, x, y, font_handle, text)
//     qp_drawtext(display, LCD_OFFSETX, LCD_OFFSETY, my_font, layer_str); 
